...
uint16_t y_padding = 50; // The padding in the y direction to not scan for corners
...
bool_t corner_detection_func(struct image_t* img)
{
  ...
  struct flow_t *vectors = opticFlowLK(&img_gray, &img_old, features, &feature_cnt, 10, 10, 10, 2, feature_cnt);
